...
function a(s) {
  ...
  const addA = (s, firstLetterIndex) => {
    const substring = s.substring(firstLetterIndex);

    if (substring.length > 0) {
      if (substring.charAt(0).toLowerCase() === 'u') {
          if (substring.length > 2) {
              if (substring.charAt(2).toLowerCase() === 'i')
                  return "a " + s;
          }
      }

      if (isVowel(substring.charAt(0))) {
          return "an " + s;
      }
    }
  ...
  return addA(s, firstLetterIndex);
}
